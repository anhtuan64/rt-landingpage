...
// Heading
if ( is_category() ) {
	echo '<div class="fusion-title title"><h3 class="title-heading-left" data-fontsize="16" data-lineheight="24">'. single_cat_title("", false) .'</h3><div class="title-sep-container"><div class="title-sep sep-single sep-solid"></div></div></div>';
}
...
echo sprintf( '<div id="posts-container" class="%sfusion-blog-archive fusion-clearfix" data-pages="%s">', $container_class, $number_of_pages );
	...
	// Start the main loop
	global $item;
	while ( have_posts() ): the_post(); $item++;
			...
			if ( $blog_layout == 'grid' ) {
				...
				if ( $blog_layout == 'grid' ||
					...
					echo '<div class="fusion-post-content post-content">';
						...
						echo '<div class="meta">';
							echo '<span class="date-time"><i class="fa fa-clock-o" aria-hidden="true"></i>'. get_the_date() .'</span>';
							$comments_count = wp_count_comments( get_the_ID() );
							echo '<span class="number-comment"><i class="fa fa-commenting-o" aria-hidden="true"></i>'. $comments_count->approved . ' ' . __( 'Comments', RT_LANGUAGE ) . '</span>';
						echo '</div>';
					...
					if ( ( Avada()->settings->get( 'post_meta' ) && ( ! Avada()->settings->get( 'post_meta_author' ) || ! Avada()->settings->get( 'post_meta_date' ) || ! Avada()->settings->get( 'post_meta_cats' ) || ! Avada()->settings->get( 'post_meta_tags' ) || ! Avada()->settings->get( 'post_meta_comments' ) || ! Avada()->settings->get( 'post_meta_read' ) ) ) ) {
						...
						echo sprintf( '<div class="read-more"><a href="%s" class="fusion-read-more"%s>%s</a></div>', get_permalink(), $link_target, apply_filters( 'avada_read_more_name', __( 'Read More', 'Avada' ) ) );
					}
				if ( $blog_layout == 'grid' ||
					 $blog_layout == 'timeline'
				) {
